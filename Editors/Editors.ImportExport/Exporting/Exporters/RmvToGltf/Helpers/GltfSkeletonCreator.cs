...
namespace Editors.ImportExport.Exporting.Exporters.RmvToGltf.Helpers
{
    ...
    {
        public static List<(Node node, Matrix4x4 invMatrix)> Create(ModelRoot model, AnimationFile animSkeletonFil)
        {
            ...
            for (var boneIndex = 0; boneIndex < animSkeletonFil.Bones.Length; boneIndex++)
            {
                ...
                parentIdToGltfNode[boneIndex].
                    WithLocalTranslation(VecConv.GetSys(GlobalSceneTransforms.FlipVector(frame.Transforms[boneIndex].ToVector3()))).
                    WithLocalRotation(VecConv.GetSys(GlobalSceneTransforms.FlipQuaternion(frame.Quaternion[boneIndex].ToQuaternion()))).
