    ...
    public class ButtonEnablement
    {
        ...
        public static void SetIsAddRowButtonEnabled(AudioEditorViewModel audioEditorViewModel, IAudioProjectService audioProjectService, IAudioRepository audioRepository)
        {
            ...
            foreach (var kvp in singleRowDataGridStringValueColumns)
            {
                ...
                if (audioEditorViewModel._selectedAudioProjectTreeItem is DialogueEvent selectedDialogueEvent && columnName != "AudioFilesDisplay")
                {
                    var stateGroup = GetStateGroupFromStateGroupWithQualifier(selectedDialogueEvent.Name, RemoveExtraUnderscoresFromString(columnName), audioProjectService.DialogueEventsWithStateGroupsWithQualifiersAndStateGroupsRepository);
