        ...
        private void WriteClusters(BinaryWriter writer)
        {
            ...
            while (videoIndex < FrameTable.Count)
            {
                ...
                while (videoIndex < FrameTable.Count)
                {
                    ...
                    var exceedsSimpleBlockTimestampRange = relativeMs > short.MaxValue;
                    var reachedTargetClusterDuration = isKeyFrame && relativeMs >= maxClusterDurationMs;
                    if (videoIndex > clusterVideoStart && (exceedsSimpleBlockTimestampRange || reachedTargetClusterDuration))
                        break;
                    ...
                    blocks.Add(new ClusterBlock(frameTimestampMs, BuildSimpleBlock(frameData, relativeTimestampMs, isKeyFrame, trackNumber: (int)TrackNumberVideo)));
                    videoIndex++;
                }
