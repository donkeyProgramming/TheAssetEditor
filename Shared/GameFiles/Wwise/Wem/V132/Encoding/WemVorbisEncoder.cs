...
namespace Shared.GameFormats.Wwise.Wem.V132.Encoding
{
    public class WemVorbisEncoder(WwiseCodebookLibrary codebookLibrary)
    {
        ...
        private const int VorbisIdPacketBlockSizeByteOffset = 28;
        private const uint WemChannelMaskMetadataBits = 0x100u;
        private const int WemChannelMaskShift = 12;
        private const uint WaveMonoChannelMask = 4u;
        private const uint WaveStereoChannelMask = 3u;
        private const uint WaveSurroundChannelMask = 63u;
        private const uint WaveFullSurroundChannelMask = 255u;
        private const int VorbisTimeDomainTransformTypeBitWidth = 16;
        private const int TerminalBitsForLongBlock = 2;
        private const int AudioPacketHeaderBitsConsumed = 1;
        private const int VorbisAsciiTagByteCount = 6;
        private const int WwiseCodebookIdBitCount = 10;
        private const int CoupleGranuleDivisor = 4;
        ...
        private readonly record struct KWeightingCoefficients(BiquadCoefficients PreFilter, BiquadCoefficients RlbFilter);

        public WemFile EncodeFromWav(byte[] wavData, WemEncodingSettings settings)
        {
            ArgumentNullException.ThrowIfNull(wavData);
            ArgumentNullException.ThrowIfNull(settings);
            var wavFile = WavFile.FromBytes(wavData);
            var perChannelSamples = ConvertPcmToPerChannelFloat(wavFile.Audio, wavFile.FmtChunk.FormatTag);
            return EncodeFloatSamplesToWem(perChannelSamples, checked(wavFile.Audio.Channels), checked((int)wavFile.Audio.SampleRate), settings);
        }

        private WemFile EncodeFloatSamplesToWem(float[][] perChannelSamples, int channels, int sampleRate, WemEncodingSettings encodingSettings)
        {
            ...
            var lastGranulePosition = wemAudioPackets.Count > 0 ? wemAudioPackets[wemAudioPackets.Count - 1].GranulePosition : sampleCount;
            ...
            if (encodingSettings.IncludeAkdChunk)
                akdChunk = BuildAkdChunk(perChannelSamples, channels, sampleRate, encodingSettings);

            var dataAlignmentBytes = 0;
            if (akdChunk != null)
                dataAlignmentBytes = 128;

            return new WemFile
            {
                DataAlignmentBytes = dataAlignmentBytes,
                FmtChunk = new FmtChunk
                {
        ...
        private static AkdChunk BuildAkdChunk(float[][] perChannelSamples, int channels, int sampleRate, WemEncodingSettings settings)
        {
            ...
            var gainDb = settings.TargetLoudnessDb - measuredLufs;
            ...
            if (!settings.IncludeHdrEnvelopeData)
                return akdChunk;
            ...
            var windowSize = Math.Max(64, settings.HdrEnvelopeWindowSizeSamples);
        ...
        private WemVorbisSetupData CompressSetup(byte[] standardSetupBytes, int channels)
        {
            ...
            for (var floorIndex = 0; floorIndex < floorCount; floorIndex++)
                CompactFloor1Configuration(input, output);
            ...
            for (var residueIndex = 0; residueIndex < residueCount; residueIndex++)
                CompactResidueConfiguration(input, output);
            ...
            for (var mappingIndex = 0; mappingIndex < mappingCount; mappingIndex++)
                CompactMappingConfiguration(input, output, channels);
        ...
        private static void SkipStandardVorbisCodebook(BitChunk input)
        {
            ...
            var quantValueCount = VorbisHelpers.ComputeMapType1QuantValues(entries, dimensions);
            for (var i = 0; i < quantValueCount; i++)
                input.ReadBits(valueLength + 1);
        }

        private static void CompactFloor1Configuration(BitChunk input, BitWriter output)
        {
            var floorType = input.ReadBits(WwiseVorbisConstants.VorbisFloorTypeBitWidth);
            if (floorType != WwiseVorbisConstants.VorbisFloorType1)
                throw new InvalidDataException($"Wwise Vorbis only supports floor type 1; encoder emitted {floorType}.");

            VorbisHelpers.TranscribeFloor1Body(input, output);
        }

        private static void CompactResidueConfiguration(BitChunk input, BitWriter output)
        {
            var residueType = input.ReadBits(WwiseVorbisConstants.VorbisResidueTypeBitWidth);
            ...
            output.WriteBits(residueType, WwiseVorbisConstants.WwiseResidueTypeBitWidth);
            VorbisHelpers.TranscribeResidueBody(input, output);
        }

        private static void CompactMappingConfiguration(BitChunk input, BitWriter output, int channelCount)
        {
            var mappingType = input.ReadBits(WwiseVorbisConstants.VorbisMappingTypeBitWidth);
            if (mappingType != WwiseVorbisConstants.VorbisMappingType0)
                throw new InvalidDataException($"Wwise Vorbis only supports mapping type 0 (got {mappingType}).");

            VorbisHelpers.TranscribeMappingBody(input, output, channelCount);
        }
