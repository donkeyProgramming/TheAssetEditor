...
{
    public class OpenPackInFileExplorerCommand(IStandardDialogs standardDialogs, IFileSystemAccess fileSystemAccess) : IContextMenuCommand
    {
        ...
        public void Execute(TreeNode _selectedNode)
        {
            ...
            {
                _logger.Here().Warning($"Open in File Explorer blocked because pack '{CommandLoggingHelper.DescribePack(_selectedNode.FileOwner)}' has not been saved to disk");
            ...
            {
                _logger.Here().Warning($"Unable to determine folder for pack '{CommandLoggingHelper.DescribePack(_selectedNode.FileOwner)}'");
