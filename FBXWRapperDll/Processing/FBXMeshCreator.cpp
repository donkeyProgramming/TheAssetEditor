...
bool wrapdll::FBXMeshCreator::MakeUnindexPackedMesh(fbxsdk::FbxScene* poFbxScene, fbxsdk::FbxMesh* poFbxMesh, PackedMesh& destMesh, const std::vector<ControlPointInfluence>& controlPointerInfluences)
{
    ...
    for (int faceIndex = 0; faceIndex < triangleCount; faceIndex++)
    {
        for (int faceCorner = 0; faceCorner < faceIndexCount; faceCorner++)
        {
            ...
            FbxVector4 v4ControlPoint = GetTransformedPosition(pControlPoints, controlPointIndex, fmGloabalTransForm);
            auto vNormalVector = GetTransformedNormal(normalMappingMode, controlPointIndex, vertexIndex, vecNormalsTable, fmGloabalTransForm_Normals);
...
FbxVector4 wrapdll::FBXMeshCreator::GetTransformedNormal(FbxGeometryElementNormal::EMappingMode NormalMappingMode, int controlPointIndex, int vertexIndex, const std::vector<fbxsdk::FbxVector4>& inputNormals, FbxAMatrix transform)
{
...
FbxVector4 wrapdll::FBXMeshCreator::GetTransformedPosition(FbxVector4* pControlPoints, int controlPointIndex, FbxAMatrix& transform)
{
