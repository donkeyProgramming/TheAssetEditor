...
template <size_t BlockSize>
static void remapVertices_and_smoothTBN(void* destination, const void* vertices, size_t vertex_count, size_t vertex_size, const unsigned int* remap)
{
	size_t block_size = BlockSize == 0 ? vertex_size : BlockSize;
	assert(block_size == vertex_size);
	...
	for (size_t i = 0; i < vertex_count; ++i)
		...
		{
			assert(remap[i] < vertex_count);            
            // TODO: vertices* -> cast to CommonPackedVertesx
            // TODO: to the TBN avaraging
...
size_t meshopt_generateVertexRemapTBN(unsigned int* destination, const unsigned int* indices, size_t index_count, PackedCommonVertex* vertices, size_t vertex_count, size_t vertex_size)
{
    ...
    std::vector<int> weightReMap(index_count);
    for (size_t i = 0; i < index_count; ++i)
	{
		unsigned int index = indices ? indices[i] : unsigned(i);
		assert(index < vertex_count);
...
void meshopt_remapVertexBuffer_andSmoothTB(void* destination, const void* vertices, size_t vertex_count, size_t vertex_size, const unsigned int* remap)
{
	using namespace meshopt;
	...
	{
	case 4:
		return remapVertices_and_smoothTBN<4>(destination, vertices, vertex_count, vertex_size, remap);

	case 8:
		return remapVertices_and_smoothTBN<8>(destination, vertices, vertex_count, vertex_size, remap);

	case 12:
		return remapVertices_and_smoothTBN<12>(destination, vertices, vertex_count, vertex_size, remap);

	case 16:
		return remapVertices_and_smoothTBN<16>(destination, vertices, vertex_count, vertex_size, remap);

	default:
		return remapVertices_and_smoothTBN<0>(destination, vertices, vertex_count, vertex_size, remap);
	}
}
