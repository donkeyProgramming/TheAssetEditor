...
namespace Audio.BnkCompiler
{
    public class AudioFileImporter
    {
        ...
        public Result<bool> ImportAudio(CompilerData compilerData)
        {
            ...
            {
                if (string.IsNullOrWhiteSpace(gameSound.SystemFilePath))
                {
                    ...
                    // Load
                    PackFileUtil.LoadFilesFromDisk(_pfs, new PackFileUtil.FileRef(wemPath.Item, $"Audio\\WWise\\{compilerData.ProjectSettings.Language}", $"{hashName}.wem"));

                    gameSound.Path = wemPath.Item;
                }
            }
